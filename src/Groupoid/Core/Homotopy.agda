  ...
  field
    .nat₁
      ...
      → S.homᵗ (G.homˢ B (F Π.$₀ a , G Π.$₀ b))
          ( G.cmpˢ B S.Π.$₀ (com₁ {b} , F Π.$₁ f)
          , G.cmpˢ B S.Π.$₀ (G Π.$₁ f , com₁ {a})
          )
open _⇒₁ᵗ_ public
...
S.homᵗ (_⇒₁ˢ_ F G) =
  λ {(α , β) → α ⇒₂ β}
com₂ (S.idnᵗ (_⇒₁ˢ_ {B = B} F G) _) = λ {a} →
  S.idnᵗ (G.homˢ B (F Π.$₀ a , G Π.$₀ a)) _
com₂ (S.cmpᵗ (_⇒₁ˢ_ {B = B} F G) (g⇒₂ , f⇒₂)) = λ {a} →
  S.cmpᵗ (G.homˢ B (F Π.$₀ a , G Π.$₀ a))
    (com₂ g⇒₂ {a} , com₂ f⇒₂ {a})
com₂ (S.invᵗ (_⇒₁ˢ_ {B = B} F G) f⇒₂) = λ {a} →
  S.invᵗ (G.homˢ B (F Π.$₀ a , G Π.$₀ a))
    (com₂ f⇒₂ {a})
...
idnˢ
  ...
  → (F : A Π.⇒₀ᵗ B)
  → S.𝟙.s S.Π.⇒₀ᵗ (F ⇒₁ˢ F)
com₁ (S.Π._$₀_ (idnˢ {B = B} F) x) =
  G.idnˢ B S.Π.$₀ x
nat₁ (S.Π._$₀_ (idnˢ {B = B} F) x) = λ {a b} f →
  S.cmpᵗ (G.homˢ B (F Π.$₀ a , F Π.$₀ b))
    ( G.idn-rhs B (F Π.$₁ f)
    , G.idn-lhs B (F Π.$₁ f) )
com₂ (S.Π._$₁_ (idnˢ {B = B} F) x) =
  G.idnˢ B S.Π.$₁ x
...
com₁ (S.Π._$₀_ (cmpˢ {B = B}) (β , α)) =
  G.cmpˢ B S.Π.$₀ (com₁ β , com₁ α)
nat₁ (S.Π._$₀_ (cmpˢ {B = B}) (β , α)) = λ f →
  S.cmpᵗ (G.homˢ B _)
  ( G.cmp-ass B _ _ _
  , S.cmpᵗ (G.homˢ B _)
    ( G.cmpˢ B S.Π.$₁
...
invˢ
  ...
  → {F G : A Π.⇒₀ᵗ B}
  → (F ⇒₁ˢ G) S.Π.⇒₀ᵗ (G ⇒₁ˢ F)
com₁ (S.Π._$₀_ (invˢ {B = B}) α) =
  G.invˢ B S.Π.$₀ (com₁ α)
com₂ (S.Π._$₁_ (invˢ {B = B}) α) =
  G.invˢ B S.Π.$₁ (com₂ α)
nat₁ (S.Π._$₀_ (invˢ {A = A}{B = B}) α) = λ f →
  S.cmpᵗ (G.homˢ B _)
    ( S.cmpᵗ (G.homˢ B _)
      ( S.cmpᵗ (G.homˢ B _)
        ( S.cmpᵗ (G.homˢ B _)
...
com₁ (S.Π._$₀_ (cmpˢ-h₀ {C = C} {K = K}) (β , α)) =
  G.cmpˢ C S.Π.$₀ (K Π.$₁ com₁ α , com₁ β)
com₂ (S.Π._$₁_ (cmpˢ-h₀ {C = C} {K = K}) (β , α)) =
  G.cmpˢ C S.Π.$₁ (K Π.$₂ com₂ α , com₂ β)
nat₁ (S.Π._$₀_ (cmpˢ-h₀ {C = C}{K = K}) (β , α)) = λ f →
  S.cmpᵗ (G.homˢ C _)
    ( S.cmpᵗ (G.homˢ C _)
      ( S.cmpᵗ (G.homˢ C _)
        ( S.cmpᵗ (G.homˢ C _)
...
cmpˢ-h₁
  ...
  → {H K : B Π.⇒₀ᵗ C}
  → ((H ⇒₁ˢ K) S.∐.⊗ (F ⇒₁ˢ G)) S.Π.⇒₀ᵗ ((H Π.∘ᵗ F) ⇒₁ˢ (K Π.∘ᵗ G))
com₁ (S.Π._$₀_ (cmpˢ-h₁ {C = C}{H = H}) (β , α)) =
  G.cmpˢ C S.Π.$₀ (com₁ β , H Π.$₁ com₁ α)
com₂ (S.Π._$₁_ (cmpˢ-h₁ {C = C}{H = H}) (β , α)) =
  G.cmpˢ C S.Π.$₁ (com₂ β , H Π.$₂ com₂ α)
nat₁ (S.Π._$₀_ (cmpˢ-h₁ {C = C}{H = H}) (β , α)) = λ f →
  S.cmpᵗ (G.homˢ C _)
    ( S.cmpᵗ (G.homˢ C _)
      ( S.cmpᵗ (G.homˢ C _)
        ( S.cmpᵗ (G.homˢ C _)
